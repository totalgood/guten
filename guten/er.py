# -*- coding: utf-8 -*-
import itertools
from sre_constants import CATEGORY_DIGIT, CATEGORY_SPACE, CATEGORY_WORD
from sre_constants import ANY, BRANCH, CATEGORY, LITERAL, IN, MAX_REPEAT, RANGE, SUBPATTERN

import sre_parse
import string


category_chars = {CATEGORY_DIGIT: string.digits,
...
def handle_any(val):
    """Unlike regexp matching this only matches printable ASCII characters."""
...
def handle_tok(tok, val):
    """Returns a list of strings of possible permutations for this regexp token."""
    handlers = {
        ANY: handle_any,
    ...
    try:
        ...
        raise ValueError("Unsupported regular expression construct: {}".format(tok))


def permute_toks(toks):
    """Generate strings, all the possible permutations of this regexp token list."""
    lists = [handle_tok(tok, val) for tok, val in toks]
    return (''.join(it) for it in join(lists))


def join(iterlist):
    """Cartesian join as an iterator of the supplied sequences.
